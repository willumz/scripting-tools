...
compile_hook() { # $1 = hook name, $2 = hook array
    ...
    if (( ${#hooks[@]} > 0 )); then
        ...
        echo "#!/bin/bash" >> "$GIT_DIR/hooks/$1"
        echo "# Generated by Hook Manager (https://github.com/willumz/scripting-tools/tree/master/git/hook-manager)" >> "$GIT_DIR/hooks/$1"
        echo "" >> "$GIT_DIR/hooks/$1"
        echo "GIT_STAGED_FILES=\$(git diff --cached --name-only)" >> "$GIT_DIR/hooks/$1"
        echo "" >> "$GIT_DIR/hooks/$1"
        ...
        for hook in "${hooks[@]}"; do
            ...
            # Insert hook name
            echo "# ---- Hook: $hook ----" >> "$GIT_DIR/hooks/$1" 
            echo "" >> "$GIT_DIR/hooks/$1"
            
            # Insert file check if on modify specified
            if [[ $HEADER_ON_MODIFY != "" ]]; then
                echo "for GIT_STAGED_FILE in \"\${GIT_STAGED_FILES[@]}\"; do" >> "$GIT_DIR/hooks/$1"
                echo "    if [[ \"\$GIT_STAGED_FILE\" =~ $HEADER_ON_MODIFY ]]; then" >> "$GIT_DIR/hooks/$1"
                echo "" >> "$GIT_DIR/hooks/$1"
            fi
            ...
            # Insert end of file check if on modify specified
            if [[ $HEADER_ON_MODIFY != "" ]]; then
                echo "    fi" >> "$GIT_DIR/hooks/$1"
                echo "done" >> "$GIT_DIR/hooks/$1"
                echo "" >> "$GIT_DIR/hooks/$1"
            fi
        done
